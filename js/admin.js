...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ pp_upload_dir+"/"+file.name + '&task=pp_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function pp_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=pp_get_albums_table', function(res)
	{
		pp_set_data_tables();
		if (res.status == 'ok') 
		{
...
function pp_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=pp_get_albums_images_table&album_id='+album_id, function(res)
	{
		pp_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'pp_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'pp_reorder_image' )
...
function pp_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'pp_bulk_delete_albums' || task == 'pp_bulk_enable_albums' || task == 'pp_bulk_disable_albums')
	{
	...
	else if( task == 'pp_bulk_delete_images' || task == 'pp_bulk_enable_images' || task == 'pp_bulk_disable_images' )
	{
...
*/
		var swfu;
			...
			var settings = {
				...
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":pp_upload_dir, "secw":pp_secw, nopar: "n" + new Date().getTime()},
	...
	jQuery('.pp_delete_album').live('click', pp_delete_album);
