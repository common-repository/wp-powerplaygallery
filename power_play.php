<?php
/*
Plugin Name: Power Play
Version: 3.3
Plugin URI: http://wpslideshow.com/powerplay-gallery/
...
define('PPLAY_PLUGIN_UPLOADS_DIR', WP_CONTENT_DIR . '/uploads/power_play');
define('PPLAY_PLUGIN_UPLOADS_URL', WP_CONTENT_URL . '/uploads/power_play');
...
/**Mobile Gallery**/
	...
	if(strstr($_SERVER['HTTP_USER_AGENT'],"iPad") || strstr($_SERVER['HTTP_USER_AGENT'],"iPhone")) {
		define('MOBILEURL', PPLAY_PLUGIN_URL);
		require_once PPLAY_PLUGIN_DIR . '/mobile/mobile-wp.php';
	}
/******************/	


class PowerPlayWidget extends WP_Widget
{
  function PowerPlayWidget()
  {
    $widget_ops = array('classname' => 'PowerPlayWidget', 'description' => 'Displays PowerPlay' );
    $this->WP_Widget('PowerPlayWidget', 'PowerPlay widget', $widget_ops);
  }
...
<?php
  ...
  function widget($args, $instance)
  {
	...
	$ret_html = display_pp_gallery($vars);
    echo $ret_html;
...
add_action( 'widgets_init', create_function('', 'return register_widget("PowerPlayWidget");') );


class PowerPlayClass
{
	...
	{
		$this->def_settings = pp_get_def_settings();
		...
		if( !is_dir(PPLAY_PLUGIN_UPLOADS_DIR) )
			$this->errors[] = __('Power Play: The uploads dir does not exists, please create "power_play" directory under "'.WP_CONTENT_DIR.' /uploads/"');
		if( is_dir(PPLAY_PLUGIN_UPLOADS_DIR) && !is_writable(PPLAY_PLUGIN_UPLOADS_DIR) )
			$this->errors[] = sprintf(__('Power Play: The upload dir "%s" is not writable, please set writte permissions'), PPLAY_PLUGIN_UPLOADS_DIR);
		if( !is_dir(PPLAY_PLUGIN_XML_DIR) )
			$this->errors[] = __('Power Play: The folder to store xml files does not exits, please create it and set write permissions');
		if( is_dir(PPLAY_PLUGIN_XML_DIR) && !is_writable(PPLAY_PLUGIN_XML_DIR) )
			$this->errors[] = sprintf(__('Power Play: The folder to store xml files "%s" is not writable, please set writte permissions'), PPLAY_PLUGIN_XML_DIR);
	}
	public function add_actions()
	{
		if( is_admin() )
		{
			...
			add_action('admin_head', array($this, 'pp_admin_head'));
		}
		else
		{
	...
	public function add_scripts()
	{
		...
		if( is_admin() && ($plugin_page == 'power_play_manage')):
			...
			wp_enqueue_style('pupload-css-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/1.8.9/themes/base/jquery-ui.css');
			wp_enqueue_style('pupload-css', PPLAY_PLUGIN_URL . '/css/jquery.ui.plupload.css');
          ...
          // wp_enqueue_script('jquery');                    // Enque Default jQuery
	       ...
	       //wp_enqueue_script('jquery-ui-tabs');            // Enque Default jQuery UI Tabs
            ...
            wp_enqueue_script('pp_pulpoade-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/1.10.2/jquery-ui.min.js', array(), '1.0.0', true  );
            wp_enqueue_script('pp_pulpoade', PPLAY_PLUGIN_URL . '/js/plupload.full.min.js', array(), '1.0.0', true  );
            wp_enqueue_script('pp_pulpoades', PPLAY_PLUGIN_URL . '/js/jquery.ui.plupload.js', array(), '1.0.0', true);

            //wp_enqueue_script( 'pulp-script', PPLAY_PLUGIN_URL . '/js/pulp.js', array(), '1.0.0', true );
		...
		if( is_admin() && ($plugin_page == 'power_play_settings')):
	...
	{
		add_shortcode('power_play', 'shortcode_display_pp_gallery');
	}
	public function add_menus()
	{
		add_menu_page(__('Power Play'), __('Power Play'), 8, 'power_play_menu', create_function('', 'require_once PPLAY_PLUGIN_DIR . "/html/about.php";'));
		add_submenu_page('power_play_menu', __('Power Play Setting'), __('About'), 8, 'power_play_menu', 
							create_function('', 'require_once PPLAY_PLUGIN_DIR . "/html/about.php";'));
		add_submenu_page('power_play_menu', __('Power Play Setting'), __('Settings'), 8, 'power_play_settings', 
							create_function('', 'require_once PPLAY_PLUGIN_DIR . "/html/settings.php";'));
		add_submenu_page('power_play_menu', __('Power Play Management'), __('Category Management'), 8, 'power_play_manage', 
							create_function('', 'require_once PPLAY_PLUGIN_DIR . "/html/manage.php";'));
		add_submenu_page('power_play_menu', __('Power Play Cache'), __('Delete Cache'), 8, 'power_play_cache', 
							create_function('', 'require_once PPLAY_PLUGIN_DIR . "/html/cache.php";'));
	}
	public function pp_admin_head()
	{
	 ...
	 */
	public function pp_add_new_album()
	{
		global $wpdb;
		...
		else
		{
			...
			$wpdb->insert($wpdb->prefix.'pp_albums', $album);
			//get album id
		...
		$wpdb->update($wpdb->prefix.'pp_albums', $album, array('album_id' => $album_id));
	 ...
	 * @return unknown_type
	 */
	public function pp_delete_album($json = true)
	{
		global $wpdb;
		...
		@rmdir($album_dir);
		//delete album
		$query = "DELETE FROM {$wpdb->prefix}pp_albums WHERE album_id = $key LIMIT 1";
		$wpdb->query($query);
		if( $json )
			$this->json_response(array('status' => 'ok', 'message' => 'The album has been deleted.'));
	}
	public function reset_albums(){update_option('pp_albums', array());delete_option('pp_albums');}
	/**
	 * Save Power Play setttings
	 * 
	 * @return unknown_type
	 */
...
insert xml code part
	...
	public function pp_single_image_upload()
	{
		global $wpdb; 
		...
		if( $image_id !== null )
		{
			...
			if( !($album = $this->pp_get_album($album_id)) )
			{
		...
		if( $image_id !== null )
		{
			...
			$data = array('title' => $title, 'description' => $desc, 'price' => $price, 
						...
						'link' => $link);
			$wpdb->update($wpdb->prefix.'pp_images', $data, array('image_id' => $image_id));
		}
		//add new image
		else
		{
			$image = array('category_id' => $album_id, 'title' => $title, 'description' => $desc,
							...
							'order' => 0, 
							'link' => '');
			if(!$wpdb->insert($wpdb->prefix.'pp_images', $image))
			{
	 ...
	 * @return unknown_type
	 */
	public function pp_resize_image_and_add()
	{
		global $wpdb;
		...
		$album = $this->pp_get_album($album_id);
		if( !$album )
		{
		...
		$wpdb->insert($wpdb->prefix.'pp_images', $image);
		...
		$row = '<tr>
					...
					<td>
						<form action="" method="post" class="order_form">
							<input type="hidden" name="task" value="pp_reorder_image" />
					...
					<td>
						<a href="'.$cpage.'&view='.$view.'&task=pp_disable_image&album_id='.$album_id.'&pp_image_id='.$image_id.'">
							'.__('Disable').'
						</a>&nbsp;
						<a class="thickbox" 
	...
	public function pp_get_albums_table()
	{
		global $wpdb;
		$query = "SELECT * FROM {$wpdb->prefix}pp_albums ORDER BY `order` ASC";
	...
	public function pp_get_albums_images_table()
	{
		$album_id = isset($_REQUEST['album_id']) ? (int)$_REQUEST['album_id'] : null;
		...
		$images = $this->pp_get_album_images($album_id);
		$cpage = 'admin.php?page=power_play_manage';
	...
	public function activate()
	{
		global $wpdb;
		$query = array();
		$query[] = "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}pp_albums(album_id bigint not null auto_increment, 
							name varchar(150), 
		...
		$query[] = "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}pp_images(image_id bigint not null auto_increment,
		...
		$query[] = "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}pp_misc(id int not null auto_increment, 
							ione int, 
							itwo int,
							ithree int,
							txt text,
		...
		$query[] = "INSERT INTO {$wpdb->prefix}pp_misc (id, ione, itwo, ithree, txt) VALUES (1,1,1,1,'".$sec_word."')";					
		foreach($query as $q)
		{
			$wpdb->query($q);
		}
	...
	{
		global $wpdb;
		//$query = "DROP TABLE {$wpdb->prefix}pp_albums";
		//$wpdb->query($query);
		//$query = "DROP TABLE {$wpdb->prefix}pp_images";
		//$wpdb->query($query);
		$query = "DROP TABLE {$wpdb->prefix}pp_misc";
		$wpdb->query($query);
	 ...
	 */
	public function pp_get_album($album_id)
	{
		global $wpdb;
	...
	public function pp_reorder_image()
	{
		global $wpdb;
		...
		$wpdb->update($wpdb->prefix.'pp_images', array('order' => $img_order), array('image_id' => $image_id));
	}
	public function pp_reorder_album()
	{
		global $wpdb;
		$album_id = (int)$_REQUEST['album_id'];
		$album_order = (int)$_REQUEST['album_order'];
		$wpdb->update($wpdb->prefix.'pp_albums', array('order' => $album_order), array('album_id' => $album_id));
	}
	public function pp_bulk_delete_albums()
	{
		global $wpdb;
	...
	public function pp_bulk_disable_albums()
	{
		$ids = json_decode($_REQUEST['ids']);
		foreach($ids as $id)
		{
			$this->pp_disable_album($id);
		}
		$this->json_response(array('status' => 'ok'));
	}
	public function pp_bulk_enable_albums()
	{
		$ids = json_decode($_REQUEST['ids']);
		foreach($ids as $id)
		{
			$this->pp_enable_album($id);
		}
		$this->json_response(array('status' => 'ok'));
	}
	public function delete_image($image_id)
	{
		...
		$wpdb->query("DELETE FROM {$wpdb->prefix}pp_images WHERE image_id = $image_id LIMIT 1");
		return true;
	}
	public function pp_bulk_delete_images()
	{
		$ids = json_decode($_REQUEST['ids']);
		$msg = '';
		foreach($ids as $id)
	...
	public function pp_bulk_disable_images()
	{
		$ids = json_decode($_REQUEST['ids']);
		foreach($ids as $id)
		{
			$this->pp_disable_image($id);
		}
		$this->json_response(array('status' => 'ok'));
	}
	public function pp_bulk_enable_images()
	{
		$ids = json_decode($_REQUEST['ids']);
		foreach($ids as $id)
		{
			$this->pp_enable_image($id);
		}
		$this->json_response(array('status' => 'ok'));
	}
	public function pp_disable_album($album_id = null)
	{
		global $wpdb;
		$album_id = (int)$album_id;
		...
		if( $album_id )
		{
			$wpdb->update($wpdb->prefix.'pp_albums', array('status' => 0), array('album_id' => $album_id));
		}
	}
	public function pp_enable_album($album_id = null)
	{
		global $wpdb;
		$album_id = (int)$album_id;
		$album_id = isset($_REQUEST['album_id']) ? (int)$_REQUEST['album_id'] : $album_id;
		if( $album_id )
			$wpdb->update($wpdb->prefix.'pp_albums', array('status' => 1), array('album_id' => $album_id));
	}
	public function pp_disable_image($image_id = null)
	{
		global $wpdb;
		$image_id = (int)$image_id;
		$image_id = isset( $_REQUEST['pp_image_id'] ) ? (int)$_REQUEST['pp_image_id'] : $image_id;
		if( $image_id)
			$wpdb->update($wpdb->prefix.'pp_images', array('status' => 0), array('image_id' => $image_id));
	}
	public function pp_enable_image($image_id = null)
	{
		global $wpdb;
		$image_id = (int)$image_id;
		$image_id = isset($_REQUEST['pp_image_id']) ? (int)$_REQUEST['pp_image_id'] : $image_id;
		if( $image_id )
			$wpdb->update($wpdb->prefix.'pp_images', array('status' => 1), array('image_id' => $image_id));
	}
}
$gpp = new PowerPlayClass();
?>